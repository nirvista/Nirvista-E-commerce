...
import 'package:pet_shop/csc_picker/select_status_model.dart';
    ...
    for (var f in states) {
      ...
      setState(() {
        ...
        for (var stateName in name) {
          stat.add(stateName.toString());
        }
      });
    }
    stat.sort((a, b) => a!.compareTo(b!));
    return stat;
  }
      ...
      cityName.forEach((ci) {
        ...
        setState(() {
          ...
          for (var cityName in citiesName) {
            city.add(cityName.toString());
          }
        });
      });
    });
    city.sort((a, b) => a!.compareTo(b!));
    return city;
  }
      ...
      children: [
        widget.layout == Layout.vertical
            ? Column(
                ...
                children: <Widget>[
                  getCustomFont("Country", 16, getFontColor(context), 1,
                      fontWeight: FontWeight.w400)
                      .marginSymmetric(horizontal: 20.h),
                  countryDropdown().marginSymmetric(horizontal: 20.h).marginOnly(bottom: 20.h,top: 6.h),
                  getCustomFont("Town", 16, getFontColor(context), 1,
                      fontWeight: FontWeight.w400)
                      .marginSymmetric(horizontal: 20.h),
                  stateDropdown().marginSymmetric(horizontal: 20.h).marginOnly(top: 6.h),
                ],
              )
            : Column(
                children: [
                  countryDropdown().marginSymmetric(horizontal: 20.h,vertical: 20.h),
                  20.h.verticalSpace,
                  widget.showStates
  ...
  Widget stateDropdown() {
    return SizedBox(
      ...
      child: DropdownWithSearch(
        ...
        disabled: stat.isEmpty ? true : false,
        items: stat.map((String? dropDownStringItem) {
          return dropDownStringItem;
        }).toList(),
        selectedItemStyle: widget.selectedItemStyle,
        dropdownHeadingStyle: widget.dropdownHeadingStyle,
  ...
  Widget cityDropdown() {
    return SizedBox(
      ...
      child: DropdownWithSearch(
        ...
        disabled: city.isEmpty ? true : false,
