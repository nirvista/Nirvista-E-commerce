...
class _ProductDetailScreen extends State<ProductDetailScreen>
  ...
  void _fetchReviews(String productId) async {
    ...
    try {
      ...
      // ✅ FIXED: Safely checking if data is list or map to avoid silent UI crashes
      if (res['success'] == true && res['data'] != null) {
        final dynamic payload = res['data'];
        final List reviewsData = payload is List ? payload : (payload['data'] is List ? payload['data'] : []);
        
        List<ReviewModel> parsedReviews = [];
        
        for (var json in reviewsData) {
          try {
            parsedReviews.add(ReviewModel.fromJson(json));
          } catch (err) {
            debugPrint('Failed to parse a review: $err');
          }
        }
        
        if (mounted) {
          setState(() {
            productReviews = parsedReviews;
          });
        }
      }
    } catch (e) {
      debugPrint('Error fetching reviews from server: $e');
    } finally {
  ...
  Widget _buildProductImage(BuildContext context, ProductModel product, double margin) {
    return Obx(() {
      selectedVariantId.value;
  ...
  Widget _buildRatingPill(BuildContext context, ProductModel product, double margin) {
    return Container(
            ...
            return SizedBox(
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final isSelected = selectedColor.value.trim().toLowerCase() == color.trim().toLowerCase();
          ...
          Obx(() {
            ...
            return Wrap(
              ...
              children: sizes.map((size) {
                final isSelected = selectedSize.value.trim().toLowerCase() == size.trim().toLowerCase();
                return GestureDetector(
                  onTap: () {
  ...
  Widget _buildBrandAndPriceSection(BuildContext context, ProductModel product, double margin) {
    return Obx(() {
      final variant = _getSelectedVariant(product);
        ...
        children: [
          Row(
            children: [
              ...
              GestureDetector(
                onTap: () {},
                child: getCustomFont("Visit Store", 12, accentColor, 1, fontWeight: FontWeight.w600),
              ),
            ],
          ),
          SizedBox(height: 8.h),
          getCustomFont(product.title, 16, getFontColor(context), 2, fontWeight: FontWeight.w700),
          SizedBox(height: 12.h),
          Row(
            children: [
              if (discountPercent > 0) ...[
                Container(
                  ...
                  child: Row(
                    children: [
                      Icon(Icons.arrow_drop_down, color: greenColor, size: 14.w),
                      getCustomFont("${discountPercent.toStringAsFixed(0)}% OFF", 11, greenColor, 1, fontWeight: FontWeight.w700),
                    ],
                  ),
                ),
                SizedBox(width: 12.w),
                getCustomFont(
                    "₹${basePrice.toStringAsFixed(0)}", 14, const Color(0xFF4B5563), 1,
                    fontWeight: FontWeight.w500, decoration: TextDecoration.lineThrough,
                    decorationColor: const Color(0xFF4B5563), decorationThickness: 2.0, txtHeight: 1.4),
              ],
            ],
          ),
          SizedBox(height: 8.h),
          getCustomFont("₹${currentPrice.toStringAsFixed(0)}", 20, accentColor, 1, fontWeight: FontWeight.w800),
          Obx(() {
            final variant = _getSelectedVariant(product);
            if (variant != null) {
  ...
  Widget _buildDeliveryDetails(BuildContext context, ProductModel product, double margin) {
    return GestureDetector(
      onTap: () {
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
                ...
                Obx(() {
                  ...
                  return Expanded(
                    child: getCustomFont(
                      ...
                      12, getFontColor(context), 1, fontWeight: FontWeight.w600, overflow: TextOverflow.ellipsis
                    ),
                  );
                }),
            ...
            Row(
              children: [
                Icon(Icons.local_shipping_outlined, color: getFontGreyColor(context), size: 16.w),
                SizedBox(width: 8.w),
                getCustomFont("Standard Delivery: 3-5 Days", 12, getFontGreyColor(context), 1, fontWeight: FontWeight.w400),
              ],
            ),
          ],
        ),
        ...
        children: List.generate(
          ...
          (i) => Expanded(
            child: Container(
              ...
              child: Column(
                children: [
                  Icon(badges[i]["icon"] as IconData, color: accentColor, size: 20.w),
                  SizedBox(height: 8.h),
                  getCustomFont(badges[i]["title"] as String, 10, getFontColor(context), 1, fontWeight: FontWeight.w600, textAlign: TextAlign.center),
                  SizedBox(height: 4.h),
                  getCustomFont(badges[i]["desc"] as String, 8, getFontGreyColor(context), 1, fontWeight: FontWeight.w400, textAlign: TextAlign.center),
                ],
              ),
            ),
          ),
  ...
  Widget _buildDescriptionSection(BuildContext context, ProductModel product, double margin) {
    return Container(
      margin: EdgeInsets.symmetric(horizontal: margin),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          getCustomFont("Description", 16, getFontColor(context), 1, fontWeight: FontWeight.w700),
          SizedBox(height: 12.h),
          getCustomFont(product.description ?? "", 14, getFontGreyColor(context), 50, fontWeight: FontWeight.w500),
        ],
      ),
    );
  }
  ...
  VariantModel? _getSelectedVariant(ProductModel product) {
    ...
    if (sColor.isNotEmpty) {
      final colorMatch = product.variants.firstWhereOrNull((v) {
        return (v.color?.trim().toLowerCase() ?? "") == sColor;
      });
    ...
    if (sSize.isNotEmpty) {
      final sizeMatch = product.variants.firstWhereOrNull((v) {
        return (v.size?.trim().toLowerCase() ?? "") == sSize;
      });
      ...
      children: [
        Padding(
          ...
          child: Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              getCustomFont("Customer Reviews", 16, getFontColor(context), 1, fontWeight: FontWeight.w700),
              getCustomFont("${productReviews.length} Reviews", 13, getFontGreyColor(context), 1, fontWeight: FontWeight.w600),
            ],
          ),
        ),
        SizedBox(height: 12.h),
            ...
            children: [
                ...
                child: isInCart
                              ...
                              children: [
                                GestureDetector(
                                  ...
                                  child: SizedBox(
                                    ...
                                    child: Center(child: Icon(Icons.remove, size: 20.w, color: getFontColor(context))),
                                  ),
                                ),
                                Container(
                                  width: 40.w, alignment: Alignment.center,
                                ...
                                GestureDetector(
                                  onTap: () {
                                    ...
                                    if (hasReachedMaxStock) {
                                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Maximum stock reached'), backgroundColor: Colors.orange));
                                      return;
                                    }
                                  ...
                                  child: SizedBox(
                                    width: 36.w, height: 36.w,
                                    child: Center(child: Icon(Icons.add, size: 20.w, color: accentColor)),
                                  ),
                                ),
                              ],
                            ),
                        ...
                        : GestureDetector(
                            ...
                            child: Container(
                              ...
                              child: Center(
                                child: getCustomFont("Add to Cart", 14, getFontColor(context), 1, fontWeight: FontWeight.w700),
                              ),
                            ),
                          ),
              ),
              ...
              Expanded(
                child: GestureDetector(
                  ...
                  child: Container(
                    padding: EdgeInsets.symmetric(vertical: 12.h),
                    decoration: BoxDecoration(color: accentColor, borderRadius: BorderRadius.circular(8.w)),
                    child: Center(
                      child: getCustomFont(
                          ...
                          Colors.white, 1, fontWeight: FontWeight.w700),
                    ),
                  ),
                ),
              ),
